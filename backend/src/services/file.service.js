...
export async function createUpload(userId, input) {
	if (
		!input.filename ||
		!input.contentType ||
		...
		throw fail("Invalid file details");
	}
	if (input.folderId) {
		await getFolder(userId, input.folderId);
	}

	const s3Key = `users/${userId}/files/${crypto.randomUUID()}/${safeName(input.filename)}`;
	const inserted = await query(
		"INSERT INTO files (user_id, folder_id, original_name, s3_key, mime_type, size) " +
		"VALUES ($1, $2, $3, $4, $5, $6) RETURNING id, s3_key",
		[
			userId,
			input.folderId || null,
			input.filename.trim(),
	...
	return {
		fileId: file.id,
		s3Key: file.s3_key,
		uploadUrl: await generateUploadUrl(file.s3_key, input.contentType)
	};
}
...
export async function complete(userId, id) {
	...
	if (!result.rowCount) {
		throw fail("File not found", 404, "NOT_FOUND");
	}
...
export async function list(userId, folderId, search, page = 1) {
	...
	// Build optional filters while keeping every value parameterized.
	if (folderId) {
...
export async function download(userId, id) {
	const file = await get(userId, id);
	return {
...
export async function update(userId, id, input) {
	...
	const result = await query(
		"UPDATE files SET original_name = COALESCE($1, original_name), " +
		"folder_id = COALESCE($2, folder_id), updated_at = NOW() " +
		"WHERE id = $3 AND user_id = $4 RETURNING *",
		[input.name?.trim() || null, input.folderId || null, id, userId]
	);
...
export async function remove(userId, id) {
	...
	// Keep the file deletion and storage accounting together.
	await query("BEGIN");
	try {
