...
export async function register(email, password) {
	if (!email || !/^\S+@\S+\.\S+$/.test(email) || !password || password.length < 8) {
		throw fail("Valid email and password of at least 8 characters are required");
	}

	const normalizedEmail = email.toLowerCase();
	const result = await query("SELECT id FROM users WHERE email = $1", [normalizedEmail]);
	if (result.rowCount) {
		throw fail("Email is already registered", 409, "EMAIL_EXISTS");
	}

	const created = await query(
		"INSERT INTO users (email, password_hash, storage_limit) " +
		"VALUES ($1, $2, $3) RETURNING *",
		[
			normalizedEmail,
			await hashPassword(password),
			Number(process.env.DEFAULT_STORAGE_LIMIT || 10737418240)
		]
	);
...
export async function login(email, password) {
	...
	const validPassword = result.rowCount && await comparePassword(
		password || "",
		result.rows[0].password_hash
	);
	if (!validPassword) {
		throw fail("Invalid email or password", 401, "INVALID_CREDENTIALS");
	}
...
export async function me(id) {
	...
	if (!result.rowCount) {
		throw fail("User not found", 404, "NOT_FOUND");
	}
